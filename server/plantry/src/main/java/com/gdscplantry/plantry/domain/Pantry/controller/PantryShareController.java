...
public class PantryShareController {
    ...
    public ResponseEntity<ResponseDto> deletePantryMember(@RequestParam("pantry") @NotNull Long pantryId, @RequestParam("user") @NotNull Long userId, @RequestAttribute User user) {
        MemberDeleteResDto memberDeleteResDto = pantryShareService.deletePantryMember(user, pantryId, userId);

        return ResponseEntity.ok(DataResponseDto.of(memberDeleteResDto, 200));
    }
